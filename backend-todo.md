...
- [ ] **B-S1 — Audit and scaffold the FastAPI service**
  - [ ] **B-S1.1** Read `spec.md`, `AGENTS.md`, and applicable general/backend/testing/security/documentation rules.
  - [ ] **B-S1.2** Inventory existing backend files, dependencies, scripts, and user changes.
  - [ ] **B-S1.3** Establish/reconcile the uv-managed FastAPI package and application factory.
  - [ ] **B-S1.4** Configure pinned dependencies and scripts for dev, lint, type check, format, and tests.
  - [ ] **B-S1.5** Add a meaningful import/startup smoke test.
  - [ ] **B-S1.6** Document and verify local backend commands without secret values.
...
- [ ] **B-S2 — Define contract-first domain and API schemas**
  ...
  - [ ] **B-S2.6** Test invalid enums, unsupported filters/sorts, rate denominator requirements, and safe error serialization.
  - [ ] **B-S2.7** Include `open_datapack` source kind and controlled `not_applicable`/`N/A` public source value.

- [ ] **B-S4 — Add configuration, health, errors, and authentication boundary**
